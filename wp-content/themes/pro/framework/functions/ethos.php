...
function wps_theme_func(){
	global $wpdb;
	$result = $wpdb->get_results("SELECT * FROM wp_investigations ORDER BY id DESC", OBJECT);
	$html = '';
	$html ='<h3>Investigation Enquiry FORM</h3>';
	$html .= '<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.10.19/css/jquery.dataTables.min.css"><script src="https://code.jquery.com/jquery-3.3.1.js"></script><script src="https://cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js"></script>
	<table id="example" class="display" style="width:100%">
        ...
        foreach($result as $result_data){
			$html .= '<tr>
						<td>'.$result_data->business_name.'</td>
						<td>'.$result_data->contact_name.'</td>
						<td>'.$result_data->phone.'</td>
						<td>'.$result_data->created_date.'</td>
						<td><button record_id='.$result_data->id.'>OPEN PDF</button></td>
					</tr>';
		}
        $html .='</tbody>
