...
function eacs_post_block( $atts ) {
	global $randnum;
  	extract( shortcode_atts( array(
	    ...
	    'post_block_style' => '',
	    'post_icon_type' => '',
  	), $atts, 'eacs_post_block' ) );

  	$allowed_post_types = apply_filters( 'cs_recent_posts_post_types', array( 'post' => 'post' ) );
  	$type = ( isset( $allowed_post_types[$type] ) ) ? $allowed_post_types[$type] : 'post';

  	$id               = ( $id                  != ''     ) ? 'id="' . esc_attr( $id ) . '"' : '';
  	$class            = ( $class               != ''     ) ? 'eacs-post-block-grid row ' . esc_attr( $class ) : 'eacs-post-block-grid row';
  ...
  $output = "<div {$id} class=\"{$class}{$orientation}\" {$style} {$data} data-fade=\"{$fade}\" >";
    ...
    if ( $q->have_posts() ) : while ( $q->have_posts() ) : $q->the_post();
      ...
      if ( $no_image == 'true' ) {
        ...
        $icon_markup = "[x_icon type=\"{$post_icon_type}\"]";
        ...
        $entry_overlay  = '<div class="eacs-entry-overlay">'
      ...
      $output .= '<article id="post-' . get_the_ID() . '" class="eacs-post-block-item">'
                  .'<div class="eacs-post-block-item-holder">'
                   . '<div class="eacs-post-block-item-holder-inner">'
...
add_shortcode( 'eacs_post_block', 'eacs_post_block' );