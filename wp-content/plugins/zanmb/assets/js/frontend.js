jQuery(document).ready(function ($) {
    ...
    $('.zmb-menu .zmb-custom-item').each(function () {
        ...
        if (zmb['has_woocommerce'] == 'yes' && thisMenuItem.is('.zmb-cart')) {
            ...
            thisMenuItem.trigger('zmb_init_mini_cart');
        }
    ...
    if ($.trim(zmb['sliding_btn_selector']) != '') {
        $(document).on('click', zmb['sliding_btn_selector'], function (e) {
            $('.zmb-sliding-content-wrap').toggleClass('active zmb-active');
            if ($('.zmb-sliding-content-wrap').is('.active') || $('.zmb-sliding-content-wrap').is('.zmb-active')) {
    ...
    if ($.trim(zmb['show_sliding_on']) != '') {
        $(document).on(zmb['show_sliding_on'], function () {
            $('.zmb-sliding-content-wrap').addClass('active zmb-active');
            $('body').addClass('zmb-sliding-content-is-opened');
        });
    }
    ...
    function zmb_close_sliding_content() {
        if ($('.zmb-sliding-content-wrap').length) {
            $('.zmb-sliding-content-wrap').removeClass('active zmb-active');
        }
        $('body').removeClass('zmb-sliding-content-is-opened');
    }
    ...
    function zmb_clone_menu_for_sticky() {
        ...
        $('.zmb-wrap.zmb-is-sticky').each(function () {
            ...
            var thisMenuContainer   = thisMenuWrap.closest('.zmb-container');
            var this_menu_id        = thisMenuWrap.find('> .zmb-menu').attr('id');
            var cloneMenu           = thisMenuWrap.clone(true);
            var zmb_container_class = thisMenuContainer.attr('class').replace('zmb-container-sticky', ''); // .zmb-container

            if (!$('.zmb-sticky-for-' + this_menu_id).length) {
                $('body').prepend('<div class="zmb-sticky-for-' + this_menu_id + ' zmb-sticky-menu-wrap"><div class="zmb-sticky-menu-inner"><div class="zmb-sticky-inner-container container"><div class="' + zmb_container_class + '"><div class="' + thisMenuWrap.attr('class') + ' zmb-menu-wrap-sticky"></div></div></div></div></div>');
            }

            cloneMenu.find('[id]').each(function () {
                // Change all tab links with href = this id
                cloneMenu.find('.vc_tta-panel-heading a[href="#' + $(this).attr('id') + '"]').attr('href', '#' + zmb_add_string_prefix($(this).attr('id'), 'zmb-sk-')); // ZMB Sticky
                cloneMenu.find('.vc_tta-tabs-list a[href="#' + $(this).attr('id') + '"]').attr('href', '#' + zmb_add_string_prefix($(this).attr('id'), 'zmb-sk-')); // ZMB Sticky
                $(this).attr('id', zmb_add_string_prefix($(this).attr('id'), 'zmb-sk-')); // ZMB Sticky
            });

            var logo_sticky_html = '';
            if (thisMenuContainer.is('.zmb-container-sticky') && thisMenuContainer.find('.zmb-logo').length) {
                var logo_sticky_link   = thisMenuContainer.find('.zmb-logo-link').attr('href');
                var logo_sticky_url    = thisMenuContainer.find('.zmb-logo-link').attr('data-href-sticky');
                var logo_sticky_width  = thisMenuContainer.find('.zmb-logo-link').attr('data-width-sticky');
                var logo_sticky_height = thisMenuContainer.find('.zmb-logo-link').attr('data-height-sticky');
                logo_sticky_html       = '<div class="zmb-logo zmb-logo-sticky"><a class="zmb-logo-sticky-link" href="' + logo_sticky_link + '"><img width="' + logo_sticky_width + '" height="' + logo_sticky_height + '" src="' + logo_sticky_url + '" alt="logo"></a></div>';
                if (!$('body .zmb-sticky-menu-wrap .zmb-container .zmb-logo-sticky').length) {
                    $('body .zmb-sticky-menu-wrap .zmb-container').prepend(logo_sticky_html);
                }
            }
    ...
    function zmb_calculate_vertical_menu_width() {
        $('.zmb-wrap.zmb-is-vertical-menu').each(function () {
            ...
            thisMenuWrap.find('.sub-menu.zanmenu').each(function () {
                ...
                var this_sub_width = thisSubMenu.attr('data-width');
                // If is number
                if (!isNaN(this_sub_width)) {
                    thisSubMenu.css({
                        'width' : this_sub_width + 'px'
                    });
                }
            });
    ...
    $('.zmb-wrap .zmb-menu .sub-menu').each(function () {
        ...
        if (typeof this_width != 'undefined' && typeof this_width != false) {
            ...
            if (!isNaN(this_width)) {
                ...
                $this.closest('.menu-item').addClass('zmb-position-relative');
            }
        }
    });
    ...
    // Ajax login
    $(document).on('submit', '.zmb-login-form', function (e) {
        ...
        if (!$('.zmb-login-form .zmb-loading').length) {
            $this.prepend('<div class="zmb-loading"><i class="fa fa-circle-o-notch fa-spin"></i></div>');
        }
